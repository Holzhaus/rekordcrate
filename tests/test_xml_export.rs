use rekordcrate::xml::{document_from_device_export, PlaylistGenericNode};
...
fn export_device_export_to_xml_document() {
    ...
    assert_eq!(document.collection.entries, 6);
    ...
    assert!(!demo_track.position_marks.is_empty());
    ...
    assert_eq!(document.playlists.node.name, "ROOT");
    assert_eq!(document.playlists.node.nodes.len(), 2);
    assert!(document
        .playlists
        .node
        .nodes
        .iter()
        .any(|node| matches!(node, PlaylistGenericNode::Playlist(playlist) if playlist.name == "Playlist1" && playlist.tracks.len() == 2)));
}
